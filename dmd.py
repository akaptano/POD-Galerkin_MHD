...
def dmd(data,r,time,M_train):
    ...
    #Bfield = np.zeros((np.shape(data)[0],tsize),dtype='complex')
    dt = 1e-6
    X = data[:,0:M_train]
    ...
    q = np.conj(np.diag(np.dot(np.dot(np.dot(Vandermonde[:,:M_train],Vdmd),np.conj(S)),Y)))
    P = np.dot(np.conj(np.transpose(Y)),Y)* \
        np.conj(np.dot(Vandermonde[:,:M_train],np.conj(VandermondeT[:M_train,:])))
    b = np.dot(np.linalg.inv(P),q)
    print(Vandermonde[:,:M_train],np.shape(Vandermonde),q,P,b)
    # plot the frequencies
    #
    c = 0.5*(b+np.conj(b)).real
    c = 500*c/np.max(c)
    energy_sort = np.flip(np.argsort(c))
    print(omega/(2*pi*1e3),c)
    plt.figure(123829312)
    omega_sizes = c
    ...
    #
    for mode in range(r):
        Bfield += 0.5*b[mode]*np.outer(Bt[:,mode],Vandermonde[mode,M_train:])
        #Bfield += (0.5*b[mode]*np.outer(Bt[:,mode],Vandermonde[mode,M_train:])).flatten()
    Bfield += np.conj(Bfield)
    return Bfield


def compare_SINDy_DMD(t_train,t_test,Q_train,sindy_Q_train,dmd_Q_train,Q_test,sindy_Q_test,dmd_Q_test):
    """
    ...
    plt.subplot(6,2,1)
    #plt.plot(t_train,Q_train[0,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[0,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,sindy_Q_train[0,:],'b',linewidth=2)
    ...
    plt.subplot(6,2,3)
    #plt.plot(t_train,Q_train[Qsize,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[1,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,sindy_Q_train[Qsize,:],'b',linewidth=2)
    ...
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.subplot(6,2,5)
    #plt.plot(t_train,Q_train[2,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[2,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,sindy_Q_train[2,:],'b',linewidth=2)
    ...
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.subplot(6,2,7)
    #plt.plot(t_train,Q_train[3,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[3,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,sindy_Q_train[3,:],'b',linewidth=2)
    ...
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.subplot(6,2,9)
    #plt.plot(t_train,Q_train[4,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[4,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,sindy_Q_train[4,:],'b',linewidth=2)
    ...
    ax.tick_params(axis='both', which='major', labelsize=18)
    plt.subplot(6,2,11)
    #plt.plot(t_train,Q_train[5,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[5,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,sindy_Q_train[5,:],'b',linewidth=2)
    ...
    #plt.plot(t_train,Q_train[0,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[0,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,dmd_Q_train[0,:],'b',linewidth=2)
    ...
    #plt.plot(t_train,Q_train[Qsize,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[1,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,dmd_Q_train[1,:],'b',linewidth=2)
    ...
    #plt.plot(t_train,Q_train[2,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[2,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,dmd_Q_train[2,:],'b',linewidth=2)
    ...
    #plt.plot(t_train,Q_train[3,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[3,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,dmd_Q_train[3,:],'b',linewidth=2)
    ...
    #plt.plot(t_train,Q_train[4,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[4,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,dmd_Q_train[4,:],'b',linewidth=2)
    ...
    #plt.plot(t_train,Q_train[5,:],'k',linewidth=2,label='True')
    plt.plot(t_test,Q_test[5,:],'k',linewidth=2,label='True')
    #plt.plot(t_train,dmd_Q_train[5,:],'b',linewidth=2)
