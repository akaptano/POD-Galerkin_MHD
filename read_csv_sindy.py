...
t_test,x_true,x_sim = compressible_Framework(inner_prod,time,poly_order,threshold,num_POD)
M_test = len(t_test)
...
tfac = 3.0/5.0
Q = ((U*S)[:,0:num_POD])@Vh[0:num_POD,int(len(time)*tfac):]
Q_true = ((U*S)[:,0:num_POD])@(np.transpose(x_true))
Q_test= ((U*S)[:,0:num_POD])@(np.transpose(x_sim))
