...
def plot_BOD_Espectrum(S):
    ...
    np.savetxt('plasmaphysics_example2_singularvalues.txt',S.real)
...
def plot_pod_temporal_modes(x,time):
    ...
    print('Maxima for mode 6: ')
    print(np.argsort(-abs(fftx[:,5]))[:12])
    print(freq[np.argsort(-abs(fftx[:,5]))[:12]])
    plot_pairwise(x)
    # now save trajectories to a file
    np.savetxt('trajectories_compressible2_modes.txt',x)
    np.savetxt('trajectories_compressible2_time.txt',time)
...
def plot_pod_spatial_modes(X,Y,Z,U):
    ...
    for i in range(6):
        for j in range(12):
            ...
            #if i==0:
            #    U_sub = np.sqrt(U[i*n_sample:(i+1)*n_sample,:]**2+U[(i+1)*n_sample:(i+2)*n_sample,:]**2)
                #U_sub = np.sqrt(U[i*n_sample:(i+1)*n_sample,:]**2+U[(i+1)*n_sample:(i+2)*n_sample,:]**2)
    ...
    np.savetxt("compressible1_spatialmodes.csv", U, delimiter=",")
# 
def plot_pairwise(x):
    """
